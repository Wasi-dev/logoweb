<?php include 'assets/layout/header.php' ?>

      <section class="banner-sec about-banner-sec pricing-page-sec">
    <div class="header-slider banner-txt-box inner-hero-banner">
    <div class="container">
        <div class="row">
            <div class="col-lg-12 col-md-12">
                <div class="anim-banner-txt" data-aos="zoom-in" data-aos-duration="2000">
                <h1>MOST AFFORDABLE <br /> PACKAGES FOR ALL <br /> OUR CUSTOMERS</h1>
                <div class="pricig-page-img-box">
                    <img src="public/assets/website/images/pricing-page-banner-img.webp" alt="" class="img-fluid" />
                </div>
                </div>
            </div>
        </div>
    </div>
    </div>
</section>
<section class="pricing-page pricing-main-sec combo-pack-sec">
    <div class="wrapv">
    <div class="vertical tlt">
        Design
    </div>
    </div>
    <div class="wrapv-two">
    <div class="vertical-two tlt-2">
        Creative.
    </div>
    </div>
    <div class="container">
    <div class="price-head-box" data-aos="fade-down" data-aos-duration="2000">
        <h2>All in One Digital Combo Packs </h2>
        <p>Our comprehensive combo packs are designed for small and medium-sized</br> enterprises in the USA seeking a holistic digital marketing approach. 
        </p>
    </div>
    <div class="pricing-tabs-box">
        <ul class="nav nav-pills combo-pricing-tabs">
            <li class="nav-item">
                <a href="#tab_content_329" data-toggle="tab" class="nav-link active">
                Basic Combo</a>
            </li>
            <li class="nav-item">
                <a href="#tab_content_330" data-toggle="tab" class="nav-link ">
                Startup Combo</a>
            </li>
            <li class="nav-item">
                <a href="#tab_content_331" data-toggle="tab" class="nav-link ">
                Professional Combo</a>
            </li>
            <li class="nav-item">
                <a href="#tab_content_332" data-toggle="tab" class="nav-link ">
                Corporate Combo</a>
            </li>
            <li class="nav-item">
                <a href="#tab_content_333" data-toggle="tab" class="nav-link ">
                Elite Combo</a>
            </li>
        </ul>
    </div>
    <div class="tab-content">
        <div id="tab_content_329" class="tab-pane in active">
            <div class="combo-pack-box" data-package-box>
                <div class="combo-pack-head">
                    <h2>BASIC COMBO</h2>
                </div>
                <div class="combo-pack-info">
                <p>Suitable for potential super-startups and brand revamps for companies</p>
                </div>
                <div class="comb-pack-desc-main-box" data-package-scroll>
                    <div class="row">
                        <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                            <div class="combo-pack-box">
                                <h3>Logo Design</h3>
                                <ul>
                                <li>5 Custom Logo Concepts</li>
                                <li>2 Dedicated Designers</li>
                                <li>Icon Design</li>
                                </ul>
                            </div>
                            <div class="combo-pack-box">
                                <h3>Features</h3>
                                <ul>
                                <li>Dedicated Account Manager</li>
                                <li>100% Satisfaction Guarantee</li>
                                <li>100% Unique Design Guarantee</li>
                                <li>100% Money Back Guarantee</li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                            <div class="combo-pack-box">
                                <h3>Branding</h3>
                                <ul>
                                <li>Business Card</li>
                                <li>Letterhead</li>
                                <li>Envelope</li>
                                <li>Fax Template</li>
                                <li>MS Word Letterhead</li>
                                </ul>
                            </div>
                            <div class="combo-pack-box">
                                <h3>Final Files</h3>
                                <ul>
                                <li>JPG</li>
                                <li>PDF</li>
                                <li>AI</li>
                                <li>PSD</li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                            <div class="combo-pack-box">
                                <h3>website</h3>
                                <ul>
                                <li>5 Page Website</li>
                                <li>Mobile Responsive</li>
                                <li>Team of Expert Designers &</li>
                                <li> Developers</li>
                                <li>8 Stock Images</li>
                                <li>15 Banner Designs</li>
                                <li>jQuery Sliders</li>
                                <li>Free Google Friendly Sitemap</li>
                                <li>Complete W3C Certified HTML</li>
                                <li>Complete Deployment</li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc-price">
                            <div class="combo-pack-price">
                                <h4>$744</h4>
                                <a data-target="#req_qoute" data-sku="BASIC COMBO ~ $744 " data-toggle="modal" class="pkg_btn btn btn-combo-view order-view-details">Order Now</a>
                            </div>
                            <div class="combo-package-chat-box">
                                <div class="chat-btn">
                                <a href="javascript:toggleTawkChat();" class="chatt">
                                    <h5>Click here to <i class="fa fa-comment"></i><span>Live Chat</span></h5>
                                </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div id="tab_content_330" class="tab-pane ">
            <div class="combo-pack-box" data-package-box>
                <div class="combo-pack-head">
                <div class="productSku" style="display: none;">STARTUP_COMBO</div>
                <h2>STARTUP COMBO</h2>
                </div>
                <div class="combo-pack-info">
                <p>Suitable for potential super-startups and brand revamps for companies</p>
                </div>
                <div class="comb-pack-desc-main-box" data-package-scroll>
                <div class="row">
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3>Logo Design</h3>
                            <ul>
                            <li>Unlimited Logo Design Concepts</li>
                            <li>3 Dedicated Designer</li>
                            <li>Social Media Icon Design</li>
                            <li>Icon Design</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Features</h3>
                            <ul>
                            <li>Dedicated Account Manager</li>
                            <li>100% Satisfaction Guarantee</li>
                            <li>100% Unique Design Guarantee</li>
                            <li>100% Money Back Guarantee</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3>Branding</h3>
                            <ul>
                            <li>Business Card</li>
                            <li>Letterhead</li>
                            <li>Envelope</li>
                            <li>Fax Template</li>
                            <li>MS Word Letterhead</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Final Files</h3>
                            <ul>
                            <li>JPG</li>
                            <li>PDF</li>
                            <li>AI</li>
                            <li>PSD</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3>website</h3>
                            <ul>
                            <li>Unlimited Pages Website</li>
                            <li>Content Management System (CMS)</li>
                            <li>Mobile Responsive</li>
                            <li> Team of Expert Designers & Developers</li>
                            <li>5 Stock Images</li>
                            <li>3 Banner Designs</li>
                            <li>Complete W3C Certified HTML</li>
                            <li>Complete Deployment</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc-price">
                        <div class="combo-pack-price">
                            <h4>$1119</h4>
                            <a data-target="#req_qoute" data-sku="STARTUP COMBO ~ $1119 " data-toggle="modal" class="pkg_btn btn btn-combo-view order-view-details">Order Now</a>
                        </div>
                        <div class="combo-package-chat-box">
                            <div class="chat-btn">
                            <a href="javascript:toggleTawkChat();" class="chatt">
                                <h5>Click here to <i class="fa fa-comment"></i><span>Live Chat</span></h5>
                            </a>
                            </div>
                        </div>
                    </div>
                </div>
                </div>
            </div>
        </div>
        <div id="tab_content_331" class="tab-pane ">
            <div class="combo-pack-box" data-package-box>
                <div class="combo-pack-head">
                <div class="productSku" style="display: none;">PROFESSIONAL_COMBO</div>
                <h2>PROFESSIONAL COMBO</h2>
                </div>
                <div class="combo-pack-info">
                <p>Suitable for potential super-startups and brand revamps for companies</p>
                </div>
                <div class="comb-pack-desc-main-box" data-package-scroll>
                <div class="row">
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3>Logo Design</h3>
                            <ul>
                            <li>Unlimited Logo Design Concepts</li>
                            <li>8 Dedicated Designer</li>
                            <li>Icon Design</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Features</h3>
                            <ul>
                            <li>Dedicated Account Manager</li>
                            <li>100% Satisfaction Guarantee</li>
                            <li>100% Unique Design Guarantee</li>
                            <li>100% Money Back Guarantee</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Final Files</h3>
                            <ul>
                            <li>JPG</li>
                            <li>PDF</li>
                            <li>AI</li>
                            <li>PSD</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3>Branding</h3>
                            <ul>
                            <li>2 Free Custom Stationery Design</li>
                            <li>Business Card</li>
                            <li>Trifold Brochure Design</li>
                            <li>Presentation Folder Design</li>
                            <li>Letterhead</li>
                            <li>Envelope</li>
                            <li>Fax Template</li>
                            <li>MS Word Letterhead</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>website</h3>
                            <ul>
                            <li>Unlimited Pages Website</li>
                            <li>Conceptual and Dynamic Liquid Website</li>
                            <li>Online Reservation/Appointment Tool </li>
                            <li>Mobile Responsive</li>
                            <li>Custom Forms</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3></h3>
                            <ul>
                            <li>Team of Expert Designers & Developers</li>
                            <li>Lead Capturing Forms</li>
                            <li>Social Media Integration</li>
                            <li>Newsfeed Integration</li>
                            <li>Search Engine Submission</li>
                            <li>15 Stock Images</li>
                            <li>8 Unique Banner Designs</li>
                            <li>jQuery Sliders</li>
                            <li>Facebook Page Design</li>
                            <li>Twitter Page Design</li>
                            <li>YouTube Page Design</li>
                            <li>Google+ Page Design</li>
                            <li>Free Google Friendly Sitemap</li>
                            <li>Complete W3C Certified HTML</li>
                            <li>Complete Deployment</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc-price">
                        <div class="combo-pack-price">
                            <h4>$1,594</h4>
                            <a data-target="#req_qoute" data-sku="PROFESSIONAL COMBO ~ $1,594 " data-toggle="modal" class="pkg_btn btn btn-combo-view order-view-details">Order Now</a>
                        </div>
                        <div class="combo-package-chat-box">
                            <div class="chat-btn">
                            <a href="javascript:toggleTawkChat();" class="chatt">
                                <h5>Click here to <i class="fa fa-comment"></i><span>Live Chat</span></h5>
                            </a>
                            </div>
                        </div>
                    </div>
                </div>
                </div>
            </div>
        </div>
        <div id="tab_content_332" class="tab-pane ">
            <div class="combo-pack-box" data-package-box>
                <div class="combo-pack-head">
                <div class="productSku" style="display: none;">CORPORATE_COMBO</div>
                <h2>CORPORATE COMBO</h2>
                </div>
                <div class="combo-pack-info">
                <p>Crafted for Born-to Lead start-ups &amp; promotion of established brands.</p>
                </div>
                <div class="comb-pack-desc-main-box" data-package-scroll>
                <div class="row">
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3>Logo Design</h3>
                            <ul>
                            <li class="first">Unlimited Logo Design Concepts</li>
                            <li>8 Dedicated Designer</li>
                            <li>Icon Design</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Features</h3>
                            <ul>
                            <li class="first">Dedicated Account Manager</li>
                            <li>100% Satisfaction Guarantee</li>
                            <li>100% Unique Design Guarantee</li>
                            <li>100% Money Back Guarantee</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Branding</h3>
                            <ul>
                            <li class="first">2 Free Custom Stationery Design</li>
                            <li>Business Card</li>
                            <li>Invoice Design</li>
                            <li>Product Catalog Design</li>
                            <li>Trifold Brochure Design</li>
                            <li>Presentation Folder Design</li>
                            <li>Letterhead </li>
                            <li>Envelope</li>
                            <li>Fax Template</li>
                            <li>MS Word Letterhead</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3>Final Files</h3>
                            <ul>
                            <li class="first">JPG</li>
                            <li>PDF</li>
                            <li>AI</li>
                            <li>PSD</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Website</h3>
                            <ul>
                            <li>Unlimited Pages Website</li>
                            <li>Conceptual and Dynamic Liquid Website</li>
                            <li>Content Management System (CMS)</li>
                            <li>Easy Product Search</li>
                            <li>Product Reviews</li>
                            <li>Unlimited Products</li>
                            <li>Unlimited Categories</li>
                            <li>Promotional Product Showcase</li>
                            <li>New Product Showcase</li>
                            <li>Full Shopping Cart Integration</li>
                            <li>Payment Module Integration</li>
                            <li>Sales & Inventory Management</li>
                            <li>Custom Forms</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3></h3>
                            <ul>
                            <li>Online Reservation/Appointment Tool </li>
                            <li>Mobile Responsive</li>
                            <li>Team of Expert Designers & Developers</li>
                            <li>Custom Forms</li>
                            <li>Lead Capturing Forms</li>
                            <li>Social Media Integration</li>
                            <li>Newsfeed Integration</li>
                            <li>Search Engine Submission</li>
                            <li>20 Stock Images</li>
                            <li>6 Unique Banner Designs</li>
                            <li>jQuery Sliders</li>
                            <li>Facebook Page Design</li>
                            <li>Twitter Page Design</li>
                            <li>YouTube Page Design</li>
                            <li>Google+ Page Design</li>
                            <li>Pinterest Page Design</li>
                            <li>Free Google Friendly Sitemap</li>
                            <li>Complete W3C Certified HTML</li>
                            <li>Complete Deployment</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc-price">
                        <div class="combo-pack-price">
                            <h4>$1755</h4>
                            <a data-target="#req_qoute" data-sku="CORPORATE COMBO ~ $1755 " data-toggle="modal" class="pkg_btn btn btn-combo-view order-view-details">Order Now</a>
                        </div>
                        <div class="combo-package-chat-box">
                            <div class="chat-btn">
                            <a href="javascript:toggleTawkChat();" class="chatt">
                                <h5>Click here to <i class="fa fa-comment"></i><span>Live Chat</span></h5>
                            </a>
                            </div>
                        </div>
                    </div>
                </div>
                </div>
            </div>
        </div>
        <div id="tab_content_333" class="tab-pane ">
            <div class="combo-pack-box" data-package-box>
                <div class="combo-pack-head">
                <div class="productSku" style="display: none;">ELITE_COMBO</div>
                <h2>ELITE COMBO</h2>
                </div>
                <div class="combo-pack-info">
                <p>Most sold website and branding package for growing businesses</p>
                </div>
                <div class="comb-pack-desc-main-box" data-package-scroll>
                <div class="row">
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3>Logo Design</h3>
                            <ul>
                            <li class="first">Unlimited Logo Design Concepts</li>
                            <li>8 Dedicated Designer</li>
                            <li>Icon Design</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Features</h3>
                            <ul>
                            <li class="first">Dedicated Account Manager</li>
                            <li>100% Satisfaction Guarantee</li>
                            <li>100% Unique Design Guarantee</li>
                            <li>100% Money Back Guarantee</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Branding</h3>
                            <ul>
                            <li class="first">2 Free Custom Stationery Design</li>
                            <li>Business Card</li>
                            <li>Invoice Design</li>
                            <li>Product Catalog Design</li>
                            <li>Trifold Brochure Design</li>
                            <li>Presentation Folder Design</li>
                            <li>Letterhead</li>
                            <li>Envelope</li>
                            <li>Fax Template</li>
                            <li>MS Word Letterhead</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3>Final Files</h3>
                            <ul>
                            <li class="first">JPG</li>
                            <li>PDF</li>
                            <li>AI</li>
                            <li>PSD</li>
                            </ul>
                        </div>
                        <div class="combo-pack-box">
                            <h3>Website</h3>
                            <ul>
                            <li>Complete Custom Website/Portal Design & Deployment</li>
                            <li>Unlimited Pages Design</li>
                            <li>Unique, User Friendly, Interactive, Dynamic, High End UI Design</li>
                            <li>Unlimited Banner Designs</li>
                            <li>JQuery Slider</li>
                            <li>Special Hover Effects</li>
                            <li>20 Stock Images</li>
                            <li>Client/User Dashboard Area</li>
                            <li>Custom Coding</li>
                            <li>Custom PHP Development</li>
                            <li>Content Management System (Custom)</li>
                            <li>Online Appointment/Scheduling/Online Ordering Integration (Optional)</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc">
                        <div class="combo-pack-box">
                            <h3></h3>
                            <ul>
                            <li>Online Payment Integration (Optional)</li>
                            <li>Multi Lingual (Optional)</li>
                            <li>Custom Dynamic Forms (Optional)</li>
                            <li>User Signup Area</li>
                            <li>User Login Area</li>
                            <li>User Dashboard Area</li>
                            <li>Complete Database Creation</li>
                            <li>Signup Automated Email Authentication</li>
                            <li>Web Traffic Analytics Integration</li>
                            <li>3rd Party API Integrations</li>
                            <li>Signup Area (For Newsletters, Offers etc.)</li>
                            <li>Search Bar for Easy Search</li>
                            <li>Live Feeds of Social Networks integration (Optional)</li>
                            <li>Search Engine Submission</li>
                            <li>Module-wise Architecture</li>
                            <li>Extensive Admin Panel</li>
                            <li>Award Winning Team of Expert Designers and Developers</li>
                            <li>Complete Deployment</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12 combo-package-desc-price">
                        <div class="combo-pack-price">
                            <h4>$4255</h4>
                            <a data-target="#req_qoute" data-sku="ELITE COMBO ~ $4255 " data-toggle="modal" class="pkg_btn btn btn-combo-view order-view-details">Order Now</a>
                        </div>
                        <div class="combo-package-chat-box">
                            <div class="chat-btn">
                            <a href="javascript:toggleTawkChat();" class="chatt">
                                <h5>Click here to <i class="fa fa-comment"></i><span>Live Chat</span></h5>
                            </a>
                            </div>
                        </div>
                    </div>
                </div>
                </div>
            </div>
        </div>
    </div>
    <a href="pricing.php" class="btn btn-view-pack">view all packages</a>
    </div>
</section>

 <?php include 'assets/layout/get-in-touch.php' ?>
 <?php include 'assets/layout/cta.php' ?>
<?php include 'assets/layout/footer.php' ?>
